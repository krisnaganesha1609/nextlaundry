...
import { titleLogo, owner, reversefatarrow } from '../../../assets'
import { useLocation, useNavigate } from 'react-router-dom'
import { ownerSidebarLinks } from '../../../model/constants'
...
const Sidebar = () => {
  ...
  const logoutHandler = async (ev) => {
    ...
    setLoading(true);
    try {
      const req = await postRequest("api/nextlaundry/owner/logout", null, `Bearer ${token}`)
      if (req.status === 200) {
        setToken("");
        setUser("");
    ...
    <>
          <nav className="bg-purple pl-6 pr-6 py-7 justify-center h-screen w-full sidebarSticky border-t-2 border-r-2 border-b-2 border-white rounded-br-xl rounded-tr-xl hidden md:flex">
              <div className="w-full my-5 gap-1 flex flex-col">
                  ...
                  <div className='flex justify-center items-center '><img src={owner} alt="" /><h5 className="font-medium text-white pt-2">Owner</h5></div>
                  

                    <div className={`bg-white container w-full h-[2px] my-3`}>
                    </div>

                  {ownerSidebarLinks.map((item, i) => (
                      <SidebarItems
                          key={i}
